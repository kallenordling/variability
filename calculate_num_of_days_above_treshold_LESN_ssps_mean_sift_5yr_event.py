...
for model in models:
	for var in var_list:
		for season in seasons:
			...
			treshold_high = ds_base.quantile(0.995,dim='time')
			ds_h = xr.where(ds_base >= treshold_high,1,0).sum('time')/(len(ds_base.time)/(31+31+30))	
			ds_h.to_dataset(name=var).to_netcdf('map_data_treshold_new/'+model+'_'+season+"_"+var+'_0_high_abs_mean_sift_5yr.nc',format="NETCDF3_64bit")
			...
			for ssp in ['126','245','370','585']:
				for times in [2025,2035,2045]:
					...
					ds_h.to_dataset(name=var).to_netcdf('map_data_treshold_new/'+model+'_'+season+"_"+var+'_'+ssp+"_"+str(times)+"_high_abs_mean_sift_5yr.nc",format="NETCDF3_64bit")
					...
					ds_h = xr.where(ds >= treshold_high,1,0).sum('time')/(len(ds.time)/(31+31+30))	
					ds_h.to_dataset(name=var).to_netcdf('map_data_treshold_new/'+model+'_'+season+"_"+var+'_'+ssp+"_"+str(times)+"_high_abs_var_5yr.nc",format="NETCDF3_64bit")
					...
					ds_h = xr.where(ds >= treshold_high,1,0).sum('time')/(len(ds.time)/(31+31+30))	
					ds_h.to_dataset(name=var).to_netcdf('map_data_treshold_new/'+model+'_'+season+"_"+var+'_'+ssp+"_"+str(times)+"_high_abs_total_5yr.nc",format="NETCDF3_64bit")
